            ...
            <div class="bg-surface-800 border border-surface-700/50 rounded-2xl p-5" x-data="{ open: false }">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        @php $activeInvoice = $project->order->invoices()->whereIn('status', ['unpaid', 'pending_confirmation', 'paid'])->first(); @endphp
                        @if($activeInvoice)
                            <p class="text-xs text-slate-500 mt-0.5">
                    ...
                    @if(!$activeInvoice)
                        <button @click="open = !open" class="px-4 py-2 bg-brand-500 hover:bg-brand-600 text-white text-sm
                ...
                <div x-show="open" x-transition class="mt-4 pt-4 border-t border-surface-700/50">
                    <form method="POST" action="{{ route('admin.projects.invoices.store', $project) }}"
                        ...
                        <div>
                            ...
                            <input type="number" name="amount" min="1000" required value="{{ $project->order->budget ?? '' }}"
                        ...
                        <div>
                            ...
                            <input type="number" name="tax" min="0" max="100" value="11" class="w-full px-3 py-2.5 bg-surface-700 border border-surface-600 text-white text-sm
                                      rounded-xl focus:outline-none focus:border-brand-500 transition-colors">
                        </div>
                        <div>
                            ...
                            <input type="date" name="due_date" required min="{{ now()->addDay()->format('Y-m-d') }}"
                                value="{{ now()->addDays(7)->format('Y-m-d') }}" class="w-full px-3 py-2.5 bg-surface-700 border border-surface-600 text-slate-300 text-sm
                                      rounded-xl focus:outline-none focus:border-brand-500 transition-colors">
                        </div>
                        <div class="sm:col-span-3">
                            <button type="submit" class="px-5 py-2.5 bg-brand-500 hover:bg-brand-600 text-white text-sm
                                       font-semibold rounded-xl transition-colors">
                                Generate Invoice
                            </button>
                        </div>
            ...
            <div class="lg:col-span-2 space-y-4">
                ...
                <div class="bg-surface-800 border border-surface-700/50 rounded-2xl p-5" x-data="{ open: false }">
                    <button @click="open = !open"
                        ...
                        <svg class="w-4 h-4 text-slate-400 transition-transform" :class="{'rotate-180': open}" fill="none"
                    ...
                    <div x-show="open" x-transition class="mt-4">
                        <form method="POST" action="{{ route('admin.projects.tasks.store', $project) }}" class="space-y-3">
                            @csrf
                            ...
                            <div class="px-5 py-4 hover:bg-surface-700/30 transition-colors">
                                <div class="flex items-start justify-between gap-3">
                                    <div class="flex-1 min-w-0">
                                        ...
                                        @if($task->description)
                                            <p class="text-slate-500 text-xs mt-0.5 line-clamp-1">{{ $task->description }}</p>
                                        @endif
            ...
            <div class="space-y-4">
                ...
                <div class="bg-surface-800 border border-surface-700/50 rounded-2xl p-5">
                    ...
                    <form method="POST" action="{{ route('admin.projects.members.add', $project) }}" class="space-y-3">
                        @csrf
                        <select name="student_id" required class="w-full px-4 py-2.5 bg-surface-700 border border-surface-600 text-slate-300
                                       text-sm rounded-xl focus:outline-none focus:border-brand-500 transition-colors">
                        ...
                        <input type="text" name="role_in_project" placeholder="Role (contoh: Frontend Dev)"
                            class="w-full px-4 py-2.5 bg-surface-700 border border-surface-600 text-white text-sm
                                      rounded-xl placeholder-slate-500 focus:outline-none focus:border-brand-500 transition-colors">
                        <button type="submit" class="w-full py-2.5 bg-brand-500 hover:bg-brand-600 text-white text-sm
                                       font-semibold rounded-xl transition-colors">
                            Tambahkan
                        </button>
                    </form>
